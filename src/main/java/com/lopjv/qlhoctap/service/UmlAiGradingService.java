...
public class UmlAiGradingService {
    ...
    public UmlSubmission analyzeAndGradeUmlSubmission(Long submissionId) {
        ...
        // If student provided PlantUML source, validate basic format and prioritize it
        if (submission.getPlantumlSource() != null && !submission.getPlantumlSource().isBlank()) {
            String src = submission.getPlantumlSource();
            if (!src.toLowerCase().contains("@startuml")) {
                submission.setAiFeedback("PlantUML source không hợp lệ: vui lòng đảm bảo bắt đầu bằng @startuml và kết thúc bằng @enduml.");
                submission.setStatus("AI_SKIPPED_INVALID_PLANTUML");
    ...
    private String buildAiEvaluationPrompt(UmlAssignment assignment, UmlSubmission submission) {
        String plantumlPart = "";
        if (submission.getPlantumlSource() != null && !submission.getPlantumlSource().isBlank()) {
            plantumlPart = "6. PlantUML source:\n" + submission.getPlantumlSource() + "\n\n";
        }
